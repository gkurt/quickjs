...
fn main() {
    ...
    // the engine is the repository this crate lives in (gkurt/quickjs), two
    // directories up, rather than a quickjs-ng submodule
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("No CARGO_MANIFEST_DIR env var is set by cargo");
    let src_dir = Path::new(&manifest_dir).join("../..");
    ...
    #[cfg(feature = "disable-typescript")]
    defines.push(("QJS_DISABLE_TYPESCRIPT".into(), None));
    ...
    if is_wasm_unknown {
        panic!(
            "wasm32-unknown-unknown is not supported by this rquickjs-sys (it would need the \
             vendored wasi-libc of upstream rquickjs-sys): use wasm32-wasip1, or the \
             @gkurt/quickjs-wasi npm package in the browser"
        );
    }
    ...
    for file in source_files.iter().chain(header_files.iter()) {
        // upstream rquickjs-sys does not watch its sources: changing the
        // engine would silently keep the previous build
        println!("cargo:rerun-if-changed={}", src_dir.join(file).display());
        fs::copy(src_dir.join(file), out_dir.join(file))
            .unwrap_or_else(|e| panic!("Unable to copy {}: {e}", src_dir.join(file).display()));
    }
